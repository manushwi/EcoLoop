...
class Dashboard {
  ...
  handleNavigation(event) {
    ...
    const targetId = event.target.closest('.nav-item').getAttribute('href').substring(1);
  ...
  async loadLeaderboardData() {
    try {
      const response = await fetch('/api/user/leaderboard?metric=carbonSaved&period=30days', {
        credentials: 'include'
      });
  ...
  createWeeklyTrendsChart() {
    ...
    if (!trends || trends.length === 0) {
      this.showChartPlaceholder('weeklyTrendsChart', 'No data available for trends');
      return;
    }
  ...
  createCategoryChart() {
    ...
    if (!categories || categories.length === 0) {
      this.showChartPlaceholder('categoryChart', 'No category data available');
      return;
    }
  ...
  createActionChart() {
    ...
    if (!stats) {
      this.showChartPlaceholder('actionChart', 'No action data available');
      return;
    }
    ...
    if (data.every(val => val === 0)) {
      this.showChartPlaceholder('actionChart', 'No actions completed yet');
      return;
    }
  ...
  createCarbonChart() {
    ...
    if (!trends || trends.length === 0) {
      this.showChartPlaceholder('carbonChart', 'No carbon data available');
      return;
    }
  ...
  updateMetrics() {
    ...
    // Total carbon saved
    const totalCarbonSavedElement = document.getElementById('totalCarbonSaved');
    if (totalCarbonSavedElement) {
      const carbonSaved = stats.totalCarbonSaved || 0;
      totalCarbonSavedElement.textContent = `${carbonSaved.toFixed(1)} kg`;
    }
  ...
  getHistoryTitle(upload) {
    ...
    const action = upload.userAction?.chosen;
    
    if (action) {
      const actionText = action.charAt(0).toUpperCase() + action.slice(1);
      return `${actionText} ${category || 'item'}`;
    }
    
    return `Analyzed ${category || 'item'}`;
  }

  // Get history subtitle
  getHistorySubtitle(upload) {
    const timeAgo = this.getTimeAgo(upload.createdAt);
    const location = upload.metadata?.location ? 'Home' : 'Unknown location';
    
    return `${timeAgo} • Location: ${location}`;
  }
  ...
  updateLeaderboard() {
    ...
    leaderboard.forEach((user, index) => {
      ...
      const score = `${user.recentStats?.carbonSaved?.toFixed(1) || 0} kg`;
    ...
    if (userRank) {
      ...
      userItem.innerHTML = `
        ...
        <span class="leaderboard-score">${this.dashboardData.stats.totalCarbonSaved?.toFixed(1) || 0} kg</span>
      `;
