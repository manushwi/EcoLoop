...
class UploadController {
  ...
  async uploadImage(req, res) {
    try {
      ...
      this.processImageAnalysis(photoUpload._id, path.join(__dirname, '..', 'uploads', fileInfo.filename), fileInfo.originalName)
        .catch(error => {
  ...
  async processImageAnalysis(uploadId, imagePath, originalName) {
    try {
      ...
      // Perform AI analysis
      const analysisResult = await ollamaService.analyzeImage(imagePath, originalName);
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          carbonSaved: photoUpload.aiAnalysis.environmental?.carbonSaved || 0
        }
      });
  ...
  async deleteUpload(req, res) {
    try {
      ...
      const filePath = path.join(__dirname, '..', 'uploads', photoUpload.filename);
      ...
      if (user && user.stats.totalUploads > 0) {
        ...
        if (photoUpload.userAction.chosen) {
          ...
          const carbonSaved = photoUpload.aiAnalysis.environmental?.carbonSaved || 0;
  ...
  async getActionRecommendations(req, res) {
    try {
      ...
      const recommendations = photoUpload.aiAnalysis.recommendations[action];
      ...
      res.json({
        ...
        data: {
          ...
          recommendations,
          environmental,
          tips: this.getActionTips(action, photoUpload.aiAnalysis.itemCategory)
        }
      });
